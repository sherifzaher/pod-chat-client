...
import {
  ...
  editMessageContent,
  resetMessageContainer, setSelectedMessage
...
export default function MessageContainer() {
  ...
  const { isEditing ,selectedMessage, messageBeingEdited } = useSelector((state: RootState) => state.messageContainer);
  ...
  const onContextMenu = useCallback((e: React.MouseEvent<HTMLDivElement, MouseEvent>, message: Message | GroupMessageType) => {
    e.preventDefault();
  ...
  const onEditMessageChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => e.key === 'Escape' && dispatch(setIsEditingMessage(false));
